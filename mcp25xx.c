...
uint8_t can_init(uint8_t opmod, uint8_t reset){
	...
	#ifdef CAN1_250KHZ
	can1_write(MCP_REG_CNF1,0xC1);								//PHSEG1 is 7xTq, PRSEG = 1xTq, SAM=1
	can1_write(MCP_REG_CNF2,0xF0);								//set BRP to 0 so we get Tq = 1/8M, then we need 16xTq to get 500kHz
	...
	#ifdef CAN3_250KHZ
	can3_write(MCP_REG_CNF1,0xC1);
	can3_write(MCP_REG_CNF2,0xF0);
	can3_write(MCP_REG_CNF3,0x06);
	#endif
...
uint8_t can123_read_rx_buf(uint8_t channel, can_frame_t* frame){
	...
	id |= (((uint32_t)(data & 0xe0)) << 13);

	if(data & 0x08){ //if IDE bit is set, we have an EID
		id |= (uint32_t) (((uint32_t)data & 0x03) << 16);
		data = spi0_write(0); // read XXXnEID8
		id |= (uint32_t) (data << 8);
		data = spi0_write(0); // read XXXnEID0
		id |= (uint32_t) data;
	} else {
